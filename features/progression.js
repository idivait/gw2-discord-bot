...
function messageReceived(message) {
	...
	if (! message.content.match(new RegExp('^'+phrases.get("CORE_PREFIX")+'('+fractal_cmd+'|'+wvw_cmd+')$', 'i'))) return;
	console.log("Progression command.");
	async.waterfall([
		function(next) { message.channel.startTyping(); db.checkKeyPermission(message.author.id, 'progression', next) },
		...
		function(result, next) {
			if (message.content.match(new RegExp('^'+phrases.get("CORE_PREFIX")+wvw_cmd+'$', 'i')))
		...
		if (message.content.match(new RegExp('^'+phrases.get("CORE_PREFIX")+fractal_cmd+'$', 'i')))
				message.reply(phrases.get("PROGRESSION_FRACTAL_LEVEL", { level: result.fractal_level }));
		else if (message.content.match(new RegExp('^'+phrases.get("CORE_PREFIX")+wvw_cmd+'$', 'i')))
